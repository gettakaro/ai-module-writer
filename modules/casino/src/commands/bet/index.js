...
import { getDefaultConfig, placeBet, settle, roundCurrency, formatCurrency, parseRouletteSelection, rouletteWin, rouletteColor, sendPlayerMessage } from './casino-helpers.js';
...
async function main() {
  ...
  await sendPlayerMessage(pog, `🎡 Spun ${spin} ${rouletteColor(spin).toUpperCase()}. ${win ? `You won ${formatCurrency(payout)} coin.` : `You lost ${formatCurrency(placed.amount)} coin.`} (Balance: ${formatCurrency(result.balance)})`);
}
