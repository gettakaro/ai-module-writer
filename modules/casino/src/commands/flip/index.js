...
async function main() {
  ...
  // ─── FLIP GAME LOGIC ─────────────────────────────────────────────────────
  if (cfg.enableFlip === false) throw new TakaroUserError('Coinflip is currently disabled.');

  const { amount, side } = data.arguments;
  const normalizedSide = (side || '').toLowerCase();
  if (normalizedSide !== 'heads' && normalizedSide !== 'tails') throw new TakaroUserError('Pick heads or tails.');

  const { effectiveHouseEdge, windowKey } = await placeBet('flip', amount);
  const edge = effectiveHouseEdge / 100;
  const flipped = Math.random() < 0.5 ? 'heads' : 'tails';
  const won = flipped === normalizedSide;
  const payout = won ? amount * 2 * (1 - edge) : 0;
  await settle('flip', amount, payout, windowKey);
  ...
  if (won) {
    await pm(`Coin flip: ${flipped.toUpperCase()}! You won ${Math.round(payout)} coins. (Balance: ${balance})`);
  } else {
    await pm(`Coin flip: ${flipped.toUpperCase()}. You lost ${amount} coins. (Balance: ${balance})`);
  }
}
await main();
