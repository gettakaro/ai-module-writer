...
async function main() {
  ...
  // ─── DICE GAME LOGIC ─────────────────────────────────────────────────────
  if (cfg.enableDice === false) throw new TakaroUserError('Dice is currently disabled.');

  const { amount, direction, target } = data.arguments;
  const dir = (direction || '').toLowerCase();
  if (dir !== 'over' && dir !== 'under') throw new TakaroUserError('Direction must be "over" or "under".');
  if (target < 2 || target > 98) throw new TakaroUserError('Target must be between 2 and 98.');

  const { effectiveHouseEdge, windowKey } = await placeBet('dice', amount);
  const edge = effectiveHouseEdge / 100;
  const roll = 1 + Math.floor(Math.random() * 100);
  const p = dir === 'over' ? (100 - target) / 100 : (target - 1) / 100;
  const multiplier = (1 - edge) / p;
  const won = dir === 'over' ? roll > target : roll < target;
  const payout = won ? amount * multiplier : 0;
  await settle('dice', amount, payout, windowKey);
  ...
  if (won) {
    await pm(`Dice: Rolled ${roll} (${dir} ${target}). Won ${Math.round(payout)} coins. (Balance: ${balance})`);
  } else {
    await pm(`Dice: Rolled ${roll} (needed ${dir} ${target}). Lost ${amount} coins. (Balance: ${balance})`);
  }
}
await main();
