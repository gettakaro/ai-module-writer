...
describe('community-fund: fund-contribute command', () => {
  ...
  it('should tell the contributor when excess contribution carries into the next round', async () => {
    ...
    assert.ok(
      logMessages.some((msg) => msg.includes('New fund total: 5/100. Completed round: 1. Next round: 2.')),
      `Expected completion feedback to report the completed round and the newly opened round, got: ${JSON.stringify(logMessages)}`,
    );
...
describe('community-fund: completion command failures', () => {
  let client: Client;
  let ctx: MockServerContext;
  let moduleId: string;
  let versionId: string;
  ...
  after(async () => {
    ...
    try {
      ...
      console.error('Cleanup: failed to uninstall completion-command-failure module:', err);
    }
    try {
      await deleteModule(client, moduleId);
    } catch (err) {
      console.error('Cleanup: failed to delete completion-command-failure module:', err);
    }
    await stopMockServer(ctx.server, client, ctx.gameServer.id);
  });

  it('logs completion command failures without failing the contribution', async () => {
    const before = new Date();
    await client.command.commandControllerTrigger(ctx.gameServer.id, {
      msg: `${prefix}fund 100`,
      playerId: ctx.players[0].playerId,
    ...
    const meta = event.meta as { result?: { success?: boolean; logs?: Array<{ msg: string }> } };
    const logMessages = (meta?.result?.logs ?? []).map((entry) => entry.msg);

    assert.equal(meta?.result?.success, true, 'Expected contribution to succeed even when a completion command fails');
    assert.ok(
      logMessages.some((msg) => msg.includes('failed to execute completion command')),
      `Expected the completion-command failure to be logged, got: ${JSON.stringify(logMessages)}`,
    );
    assert.ok(
      logMessages.some((msg) => msg.includes('Completed round: 1. Next round: 2.')),
      `Expected the player success path to report both the completed round and the next active round after the completion-command failure, got: ${JSON.stringify(logMessages)}`,
    );
  });
});
...
describe('community-fund: refund handling after state-write failure', () => {
  ...
  it('warns the player when refund succeeds but shared-state restore still fails', async () => {
    const player = ctx.players[0]!;
    await client.variable.variableControllerCreate({
      key: '__debug_force_state_write_failure_after_deduct',
      value: JSON.stringify(true),
    ...
    await client.variable.variableControllerCreate({
      key: FUND_DEBUG_FORCE_STATE_RESTORE_FAILURE_KEY,
      value: JSON.stringify(true),
      gameServerId: ctx.gameServer.id,
      moduleId,
    });
    ...
    const beforePog = await getPog(player.playerId);
    assert.ok(beforePog, 'Expected player POG before restore-failure test');
    const beforeCurrency = beforePog?.currency ?? 0;
    const before = new Date();
    ...
    const meta = event.meta as { result?: { success?: boolean; logs?: Array<{ msg: string }> } };
    assert.equal(meta?.result?.success, false, 'Expected contribution to fail after the forced restore failure');
    ...
    assert.ok(
      logs.some((msg) => msg.includes('CRITICAL shared-state restore failure')),
      `Expected shared-state restore failure to be logged, got: ${JSON.stringify(logs)}`,
    );
    assert.ok(
      logs.some((msg) => msg.includes('currency was refunded, but the community fund state could not be restored cleanly')),
      `Expected player-facing restore-failure warning, got: ${JSON.stringify(logs)}`,
    );

    const afterPog = await getPog(player.playerId);
    assert.ok(afterPog, 'Expected player POG after restore-failure test');
    assert.equal(afterPog?.currency, beforeCurrency, `Expected refunded currency balance after restore failure, got: ${JSON.stringify(afterPog)}`);
  });
