...
import { cleanupTestGameServers, cleanupTestModules, deleteModule, installModule, uninstallModule, pushModule, } from '../../../test/helpers/modules.js';
...
function cronExpressionForTimeZoneNow(timeZone) {
    const weekdayMap = {
        Sun: '0',
        Mon: '1',
        Tue: '2',
...
describe('server-messages integration', () => {
    ...
    async function reinstall(userConfig) {
        if (!versionId) {
    ...
    async function clearModuleVariables() {
        ...
        await Promise.all(result.data.data.map(async (variable) => {
            try {
    ...
    async function waitForOnlineCount(expected, timeout = 30000) {
        const start = Date.now();
        while (Date.now() - start < timeout) {
            if ((await getOnlineCount()) === expected)
                return;
            await wait(250);
        }
        throw new Error(`Timed out waiting for ${expected} online players`);
    }
    async function readModuleVariable(key) {
        const result = await client.variable.variableControllerSearch({
            filters: {
                key: [key],
                gameServerId: [ctx.gameServer.id],
    ...
    async function upsertModuleVariable(key, value) {
    ...
    async function waitForEvents(eventName, after, count = 1, timeout = 15000) {
        const deadline = Date.now() + timeout;
        while (Date.now() < deadline) {
    ...
    async function assertNoEvent(eventName, after) {
        await wait(250);
        const result = await client.event.eventControllerSearch({
            filters: {
                eventName: [eventName],
        ...
        assert.equal(result.data.data.length, 0, `Expected no '${eventName}' events after ${after.toISOString()}, got ${JSON.stringify(result.data.data)}`);
    }
    function parseJsonLogField(logs, field) {
    ...
    function parseNumericLogField(logs, field) {
        const line = logs.find((entry) => entry.includes(`${field}=`));
        if (!line)
            return null;
    ...
    it('allows empty message lists to install quietly, warns about unknown placeholders, and leaves them unchanged', async () => {
        ...
        assert.ok(unknownPlaceholderRun.logs.some((log) => log.includes('unsupported placeholders') && log.includes('unknown')), `Expected unknown-placeholder run to log a warning, logs: ${JSON.stringify(unknownPlaceholderRun.logs)}`);
        ...
        await safeUninstall();
        await assert.rejects(installModule(client, versionId, ctx.gameServer.id, {
            userConfig: {
                messages: [{ text: '   ' }],
                order: 'sequential',
                interval: '* * * * *',
        ...
        for (const interval of ['not-a-cron', '61 * * * *', '*/99 * * * *', '* * 32 * *', '5-1 * * * *']) {
            ...
            }), /400|pattern|bad request|validation/i, `Expected install-time validation to reject interval ${interval}`);
        }
    });
    ...
    it('serializes overlapping executions so overlapping triggers do not duplicate the same broadcast', async () => {
        ...
        const cronLogs = cronEvents.map((event) => event.meta.result?.logs?.map((log) => log.msg) ?? []);
        assert.equal(cronLogs.filter((logs) => logs.some((log) => log.includes('another execution already holds the rotation lock'))).length, 1);
    ...
    it('consumes each weighted bag slot exactly once before rebuilding and no longer logs the full bag', async () => {
        ...
        const stateAfterFirst = JSON.parse((await readModuleVariable(STATE_KEY)).value);
        ...
        assert.equal(sentIndices.filter((entry) => entry === 0).length, 1, `Expected exactly one Red slot per bag, got sentIndices=${JSON.stringify(sentIndices)} bag=${JSON.stringify(bag)}`);
        assert.equal(sentIndices.filter((entry) => entry === 1).length, 2, `Expected exactly two Gold slots per bag, got sentIndices=${JSON.stringify(sentIndices)} bag=${JSON.stringify(bag)}`);
        const fourth = await triggerCronjob();
        assert.equal(fourth.success, true);
        const stateAfterFourth = JSON.parse((await readModuleVariable(STATE_KEY)).value);
    ...
    it('recovers from malformed persisted state for sequential and random rotation paths', async () => {
        ...
        const recoveredState = JSON.parse((await readModuleVariable(STATE_KEY)).value);
    ...
    it('restarts rotation cleanly after config changes', async () => {
        ...
        assert.equal(parseJsonLogField(beforeChange.logs, 'rendered'), 'Old one', `Expected initial render log to be Old one, got: ${JSON.stringify(beforeChange.logs)}`);
        await safeUninstall();
        await installModule(client, versionId, ctx.gameServer.id, {
            userConfig: {
                messages: [
    ...
    it('releases the execution lock when broadcasting fails after the lock is acquired', async () => {
        ...
        assert.deepEqual(JSON.parse(stateAfterFailure.value), { order: 'sequential', index: 0 }, 'Expected failed sends to roll rotation state back so the skipped message is retried later');
    });
});
