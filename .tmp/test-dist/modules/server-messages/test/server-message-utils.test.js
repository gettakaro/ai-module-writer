...
import { createExecutionLockHeartbeat, findUnknownPlaceholders, getIntervalStatus, normalizeMessages, renderPlaceholders, } from '../src/functions/server-message-utils.js';
describe('server-message utils', () => {
    ...
    it('finds unsupported placeholders and leaves them unchanged when rendering', () => {
        ...
        assert.equal(renderPlaceholders('Hello {serverName} {unknown} ({playerCount})', { serverName: 'Paper One', playerCount: 3 }), 'Hello Paper One {unknown} (3)');
    });
    ...
    it('schedules heartbeats, deduplicates concurrent refreshes, and stops rescheduling after failure', async () => {
        ...
        let releaseRefresh;
        const heartbeat = createExecutionLockHeartbeat(async () => {
            refreshCalls += 1;
