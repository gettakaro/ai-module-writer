...
import { pushModule, installModule, uninstallModule, deleteModule, cleanupTestModules, cleanupTestGameServers, assignPermissions, cleanupRole, } from '../../../test/helpers/modules.js';
...
describe('afk-kick: check-afk cronjob', () => {
    ...
    before(async () => {
        ...
        if (!cronjob)
            throw new Error('Expected at least one cronjob in afk-kick module');
        cronjobId = cronjob.id;
        assert.ok(ctx.players[2], 'Expected at least 3 players in mock server (players[2] must exist)');
        immuneRoleId = await assignPermissions(client, ctx.players[2].playerId, ctx.gameServer.id, ['IMMUNE_TO_AFK_KICK']);
    ...
    it('first check stores positions without warnings', async () => {
        ...
        assert.ok(!logs.some((msg) => msg.toLowerCase().includes('warned')), `Expected no warnings on first check, got: ${JSON.stringify(logs)}`);
        assert.ok(!logs.some((msg) => msg.toLowerCase().includes('kicked')), `Expected no kicks on first check, got: ${JSON.stringify(logs)}`);
    });
    it('second check increments idle count, no warning yet', async () => {
        ...
        const { success, logs } = await triggerCronjob();
        assert.equal(success, true, `Expected cronjob to succeed, logs: ${JSON.stringify(logs)}`);
        assert.ok(!logs.some((msg) => msg.toLowerCase().includes('warned')), `Expected no warnings on second check (idleCount=1 < checksBeforeWarning=2), got: ${JSON.stringify(logs)}`);
        assert.ok(!logs.some((msg) => msg.toLowerCase().includes('kicked')), `Expected no kicks on second check, got: ${JSON.stringify(logs)}`);
    });
    it('warns after checksBeforeWarning checks', async () => {
        // checksBeforeWarning=2, idleCount will hit 2 on this trigger
        const { success, logs } = await triggerCronjob();
        assert.equal(success, true, `Expected cronjob to succeed, logs: ${JSON.stringify(logs)}`);
        assert.ok(logs.some((msg) => msg.toLowerCase().includes('warned')), `Expected a warning log on third check (idleCount=2 >= checksBeforeWarning=2), got: ${JSON.stringify(logs)}`);
    });
    it('kicks after checksBeforeKick checks', async () => {
        // checksBeforeKick=3, idleCount will hit 3 on this trigger
        const { success, logs } = await triggerCronjob();
        assert.equal(success, true, `Expected cronjob to succeed, logs: ${JSON.stringify(logs)}`);
        assert.ok(logs.some((msg) => msg.toLowerCase().includes('kicked')), `Expected a kick log on fourth check (idleCount=3 >= checksBeforeKick=3), got: ${JSON.stringify(logs)}`);
        ...
        assert.ok(atLeastOneKicked, 'Expected at least one non-immune player to be offline after kick trigger');
    });
    it('immune player is not kicked', async () => {
        ...
        const { success, logs } = await triggerCronjob();
        assert.equal(success, true, `Expected cronjob to succeed, logs: ${JSON.stringify(logs)}`);
        assert.ok(logs.some((msg) => msg.toLowerCase().includes('immune')), `Expected a log mentioning "immune" for player[2], got: ${JSON.stringify(logs)}`);
        ...
        assert.equal(pog.online, true, `Expected immune player[2] to still be online after kick trigger, but online=${pog.online}`);
    });
});
