...
function makeCommandHelpers(getClient, getGameServerId, getPrefix) {
    async function triggerCommand(playerId, msg) {
...
function makeCronjobHelper(getClient, getGameServerId, getCronjobId, getModuleId) {
...
describe('vote-restart module', () => {
    ...
    before(async () => {
        ...
        if (!cronjob)
            throw new Error('Expected at least one cronjob in vote-restart module');
        cronjobId = cronjob.id;
        // players[0] gets VOTE_RESTART_INITIATE
        initiateRoleId = await assignPermissions(client, ctx.players[0].playerId, ctx.gameServer.id, ['VOTE_RESTART_INITIATE']);
        // players[2] gets VOTE_RESTART_IMMUNE
        immuneRoleId = await assignPermissions(client, ctx.players[2].playerId, ctx.gameServer.id, ['VOTE_RESTART_IMMUNE']);
    });
    after(async () => {
    ...
    const { triggerCommand, getResult } = makeCommandHelpers(() => client, () => ctx.gameServer.id, () => prefix);
    const triggerCronjob = makeCronjobHelper(() => client, () => ctx.gameServer.id, () => cronjobId, () => moduleId);
    ...
    it('should start a vote when player has VOTE_RESTART_INITIATE', async () => {
        ...
        assert.ok(logs.some((l) => l.includes('vote started')), `Expected log to mention "vote started", got: ${JSON.stringify(logs)}`);
    });
    ...
    it('should reject starting a vote when one is already active', async () => {
        ...
        assert.ok(logs.some((l) => l.includes('already in progress')), `Expected "already in progress" in logs, got: ${JSON.stringify(logs)}`);
    });
    ...
    it('should reject duplicate /voteyes from same player', async () => {
        ...
        const { success, logs } = getResult(event);
        assert.equal(success, false, `Expected success=false, logs: ${JSON.stringify(logs)}`);
        assert.ok(logs.some((l) => l.includes('already voted')), `Expected "already voted" in logs, got: ${JSON.stringify(logs)}`);
    });
    ...
    it('should reject /voteyes from an immune player', async () => {
        ...
        const { success, logs } = getResult(event);
        assert.equal(success, false, `Expected success=false, logs: ${JSON.stringify(logs)}`);
        assert.ok(logs.some((l) => l.includes('immune')), `Expected "immune" in logs, got: ${JSON.stringify(logs)}`);
    });
    ...
    it('should reject /voterestart without VOTE_RESTART_INITIATE permission', async () => {
        ...
        const { success, logs } = getResult(event);
        assert.equal(success, false, `Expected success=false, logs: ${JSON.stringify(logs)}`);
        assert.ok(logs.some((l) => l.includes('do not have permission')), `Expected "do not have permission" in logs, got: ${JSON.stringify(logs)}`);
    });
    ...
    it('should show active vote status with count and time remaining', async () => {
        ...
        assert.ok(logs.some((l) => l.includes('1/2') || (l.includes('vote') && l.includes('remaining'))), `Expected vote count and time remaining in logs, got: ${JSON.stringify(logs)}`);
    });
    ...
    it('should accept /voteyes from a non-immune player and pass the vote', async () => {
        ...
        const { success, logs } = getResult(event);
        assert.equal(success, true, `Expected success=true, logs: ${JSON.stringify(logs)}`);
        assert.ok(logs.some((l) => l.includes('voted yes')), `Expected "voted yes" in logs, got: ${JSON.stringify(logs)}`);
        // The vote should immediately pass since we hit the threshold
        assert.ok(logs.some((l) => l.includes('Vote passed')), `Expected "Vote passed" log, got: ${JSON.stringify(logs)}`);
    });
    ...
    it('should show passed vote status after vote passes', async () => {
        ...
        const { success, logs } = getResult(event);
        assert.equal(success, true, `Expected success=true, logs: ${JSON.stringify(logs)}`);
        assert.ok(logs.some((l) => l.includes('restart already initiated') || l.includes('restarting in')), `Expected "restart already initiated" or "restarting in" in logs, got: ${JSON.stringify(logs)}`);
    });
    ...
    it('should execute the restart command when restartDelay has elapsed', async () => {
        ...
        assert.ok(logs.some((l) => l.includes('restart command executed successfully')), `Expected "restart command executed successfully" in cronjob logs, got: ${JSON.stringify(logs)}`);
    });
    ...
    it('should reject /voteyes when no vote is active', async () => {
        ...
        const { success, logs } = getResult(event);
        assert.equal(success, false, `Expected success=false, logs: ${JSON.stringify(logs)}`);
        assert.ok(logs.some((l) => l.includes('no active restart vote')), `Expected "no active restart vote" in logs, got: ${JSON.stringify(logs)}`);
    });
    ...
    it('should show "No active restart vote" when no vote is in progress', async () => {
        ...
        const { success, logs } = getResult(event);
        assert.equal(success, true, `Expected success=true, logs: ${JSON.stringify(logs)}`);
        assert.ok(logs.some((l) => l.includes('No active restart vote') || l.includes('no active restart vote')), `Expected "No active restart vote" in logs, got: ${JSON.stringify(logs)}`);
    });
    ...
    it('should enforce cooldown after an expired vote', async () => {
        ...
        assert.ok(cjLogs.some((l) => l.includes('expired') || l.includes('vote expired')), `Expected "expired" in cronjob logs, got: ${JSON.stringify(cjLogs)}`);
        ...
        assert.ok(logs.some((l) => l.includes('recently failed') || l.includes('wait')), `Expected cooldown message in logs, got: ${JSON.stringify(logs)}`);
    });
});
...
describe('vote-restart edge cases', () => {
    ...
    before(async () => {
        ...
        // players[0] gets VOTE_RESTART_INITIATE
        initiateRoleId2 = await assignPermissions(client2, ctx2.players[0].playerId, ctx2.gameServer.id, ['VOTE_RESTART_INITIATE']);
        // players[2] gets VOTE_RESTART_IMMUNE
        immuneRoleId2 = await assignPermissions(client2, ctx2.players[2].playerId, ctx2.gameServer.id, ['VOTE_RESTART_IMMUNE']);
    });
    after(async () => {
    ...
    const { triggerCommand: triggerCommand2, getResult: getResult2 } = makeCommandHelpers(() => client2, () => ctx2.gameServer.id, () => prefix2);
    ...
    it('should reject /voterestart when fewer than minimumPlayers non-immune players are online', async () => {
        ...
        assert.equal(success, false, `Expected success=false, logs: ${JSON.stringify(logs)}`);
        assert.ok(logs.some((l) => l.includes('Not enough players')), `Expected "Not enough players" in logs, got: ${JSON.stringify(logs)}`);
    });
    ...
    it('should reject /voteyes when the vote has already passed', async () => {
        ...
        assert.ok(logs.some((l) => l.includes('already passed') || l.includes('Waiting for restart')), `Expected "already passed" or "Waiting for restart" in logs, got: ${JSON.stringify(logs)}`);
    });
});
...
describe('vote-restart dynamic threshold recalculation', () => {
    ...
    const triggerCronjob3 = makeCronjobHelper(() => client3, () => ctx3.gameServer.id, () => cronjobId3, () => moduleId3);
    ...
    it('should detect vote pass when dynamic threshold recalculates to match effective votes', async () => {
        ...
        try {
            ...
            assert.equal(success, true, `Expected cronjob to succeed, logs: ${JSON.stringify(logs)}`);
            assert.ok(logs.some((l) => l.includes('Vote passed')), `Expected "Vote passed" in cronjob logs (dynamic threshold recalculation), got: ${JSON.stringify(logs)}`);
        }
        finally {
