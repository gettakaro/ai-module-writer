...
describe('lottery: draw-lottery cronjob', () => {
    ...
    before(async () => {
        ...
        if (!cronjob)
            throw new Error('Expected at least one cronjob in lottery module');
        ...
        buy2RoleId = await assignPermissions(client, ctx.players[1].playerId, ctx.gameServer.id, ['LOTTERY_BUY']);
        ...
        await client.playerOnGameserver.playerOnGameServerControllerAddCurrency(ctx.gameServer.id, ctx.players[1].playerId, { currency: 500 });
    });
    after(async () => {
    ...
    it('should cancel draw and refund exact ticket price when not enough participants', async () => {
        ...
        assert.ok(logs.some((msg) => msg.includes('cancelling draw')), `Expected cancellation log, got: ${JSON.stringify(logs)}`);
        ...
        assert.equal(currencyAfterRefund, currencyBeforeRefund + 10, `Expected player currency to increase by exactly 10 (refund). Before: ${currencyBeforeRefund}, After: ${currencyAfterRefund}`);
    });
    it('should run draw and award winner exact prize when enough participants', async () => {
        ...
        assert.ok(logs.some((msg) => msg.includes('draw-lottery: winner=')), `Expected winner selection log, got: ${JSON.stringify(logs)}`);
        assert.ok(logs.some((msg) => msg.includes('drawNumber=')), `Expected drawNumber increment log, got: ${JSON.stringify(logs)}`);
        ...
        assert.ok(player0Won || player1Won, `Expected one player to receive exact prize of ${expectedPrize}. Player0 gain: ${player0Gain}, Player1 gain: ${player1Gain}`);
        // Only one player should have won
        assert.ok(!(player0Won && player1Won), `Expected only one winner, but both players gained ${expectedPrize}`);
    });
});
