...
describe('lottery: /buyticket command', () => {
    ...
    before(async () => {
        ...
        await client.playerOnGameserver.playerOnGameServerControllerAddCurrency(ctx.gameServer.id, ctx.players[1].playerId, { currency: 100 });
    });
    after(async () => {
    ...
    it('should buy 1 ticket successfully', async () => {
        ...
        assert.ok(logMessages.some((msg) => msg.includes('Lottery:') && msg.includes('buying 1 tickets')), `Expected log to contain "buying 1 tickets", got: ${JSON.stringify(logMessages)}`);
        ...
        assert.equal(currencyAfter, currencyBefore - 10, `Expected currency to decrease by 10 (ticket price). Before: ${currencyBefore}, After: ${currencyAfter}`);
    });
    it('should buy multiple tickets with correct additional currency deduction', async () => {
        ...
        const logMessages = (meta?.result?.logs ?? []).map((l) => l.msg);
        assert.ok(logMessages.some((msg) => msg.includes('Lottery:') && msg.includes('buying 2 tickets')), `Expected log to show buying 2 tickets, got: ${JSON.stringify(logMessages)}`);
        // Verify cumulative ticket count is now 3
        assert.ok(logMessages.some((msg) => msg.includes('newTickets=3')), `Expected newTickets=3 in log, got: ${JSON.stringify(logMessages)}`);
        ...
        const currencyAfter = pogAfter.data.data[0]?.currency ?? 0;
        assert.equal(currencyAfter, currencyBefore - 20, `Expected currency to decrease by 20 (2 tickets * price 10). Before: ${currencyBefore}, After: ${currencyAfter}`);
    });
    ...
    it('should reject purchase exceeding maxTicketsPerPlayer', async () => {
        ...
        const logMessages = (meta?.result?.logs ?? []).map((l) => l.msg);
        assert.ok(logMessages.some((msg) => msg.includes('only hold') || msg.includes('maxTicketsPerPlayer') || msg.includes('5 tickets')), `Expected max tickets error, got: ${JSON.stringify(logMessages)}`);
    });
    it('should reject purchase when player has insufficient currency', async () => {
        ...
        if (currentCurrency > 0) {
            await client.playerOnGameserver.playerOnGameServerControllerDeductCurrency(ctx.gameServer.id, ctx.players[0].playerId, { currency: currentCurrency });
        }
        // Give exactly 10: enough to pass the command cost check but not enough for 2 tickets total.
        await client.playerOnGameserver.playerOnGameServerControllerAddCurrency(ctx.gameServer.id, ctx.players[0].playerId, { currency: 10 });
        const player = ctx.players[0];
        const before = new Date();
        ...
        const logMessages = (meta?.result?.logs ?? []).map((l) => l.msg);
        assert.ok(logMessages.some((msg) => msg.includes('enough currency')), `Expected "enough currency" error message, got: ${JSON.stringify(logMessages)}`);
    });
});
