...
describe('lottery: /viewtickets command', () => {
    ...
    before(async () => {
        ...
        permRoleId = await assignPermissions(client, ctx.players[0].playerId, ctx.gameServer.id, ['LOTTERY_BUY', 'LOTTERY_VIEW_TICKETS']);
    ...
    it('should show zero tickets before any purchase', async () => {
        ...
        const logMessages = (meta?.result?.logs ?? []).map((l) => l.msg);
        assert.ok(logMessages.some((msg) => msg.includes('tickets=0')), `Expected log to show tickets=0, got: ${JSON.stringify(logMessages)}`);
    });
    it('should show correct ticket count after purchase', async () => {
        ...
        const logMessages = (meta?.result?.logs ?? []).map((l) => l.msg);
        assert.ok(logMessages.some((msg) => msg.includes('tickets=2')), `Expected log to show tickets=2, got: ${JSON.stringify(logMessages)}`);
    });
});
