...
async function retry(fn, maxAttempts, delayMs, label) {
    for (let i = 0; i < maxAttempts; i++) {
        try {
            ...
            console.error(`Retry ${i + 1}/${maxAttempts}${context}: ${err.message}, retrying in ${delayMs}ms...`);
            await wait(delayMs);
        }
    }
    throw new Error('Retry exhausted');
}
export async function startMockServer(client) {
    const registrationToken = process.env['TAKARO_REGISTRATION_TOKEN'];
    const wsUrl = process.env['TAKARO_WS_URL'];
    if (!registrationToken)
        throw new Error('TAKARO_REGISTRATION_TOKEN is required');
    if (!wsUrl)
        throw new Error('TAKARO_WS_URL is required');
    ...
    try {
        ...
        const gameServer = await retry(async () => {
            ...
            if (!found)
                throw new Error(`Game server with identityToken ${identityToken} not found yet`);
            return found;
        }, 30, 2000, 'discover game server');
        ...
        const players = await retry(async () => {
            ...
            if (found.length < population.totalPlayers)
                throw new Error(`Waiting for ${population.totalPlayers} players to be online, only ${found.length} found so far`);
            return found;
        }, 20, 2000, 'wait for players');
...
function forceStopWsClient(wsClient) {
    ...
    if (wsClient.ws) {
        try {
            ...
            console.error('forceStopWsClient: ws.terminate() failed (safe to ignore):', terminateErr);
        }
        wsClient.ws = null;
    }
}
export async function stopMockServer(server, client, gameServerId) {
    ...
    try {
        ...
        const wsClient = server.wsClient;
        if (wsClient)
            forceStopWsClient(wsClient);
    }
    catch (err) {
