...
export async function createClient() {
    ...
    if (!url)
        throw new Error('TAKARO_HOST is required');
    if (!username)
        throw new Error('TAKARO_USERNAME is required');
    if (!password)
        throw new Error('TAKARO_PASSWORD is required');
    if (!domainId)
        throw new Error('TAKARO_DOMAIN_ID is required');
